...
% [row col]        = size(im);
% block_rownum     = floor(row/blocksizerow); %% 5行
% block_colnum     = floor(col/blocksizecol); %% 6列
% im               = im(1:block_rownum*blocksizerow,1:block_colnum*blocksizecol);               
...
feat_scale               = blkproc(structdis,[blocksizerow/itr_scale blocksizecol/itr_scale], ...
                           ...
                           @computefeature);
feat_scale               = reshape(feat_scale,[featnum,size(feat_scale,1)*size(feat_scale,2)/featnum]);  %%特征矩阵变为18*2 18行2列
feat_scale               = feat_scale'; %输入的特征矩阵为2*18 2行18列


if(itr_scale == 1)
sharpness                = blkproc(sigma,[blocksizerow blocksizecol], ...
                           [blockrowoverlap blockcoloverlap],@computemean);
sharpness                = sharpness(:);  %%没有筛选图像块的过程
end
...
distparam        = feat;    %30*36
mu_distparam     = nanmean(distparam);  %求均值  1*36
cov_distparam    = nancov(distparam);   %求方差  36*36
